...
const NeumorphicToggleSection = ({ title, icon, data }) => {
    const [expanded, setExpanded] = useState(false);
    const visibleItems = expanded ? data : data.slice(0, 1);

    return (
        <div className="bg-[#f0f0f3] rounded-2xl p-6 shadow-[6px_6px_12px_#d1d9e6,-6px_-6px_12px_#ffffff] transition-all h-full flex flex-col">
            ...
            <div className="space-y-4 flex-1 overflow-y-auto max-h-[400px] pr-1">
                {visibleItems.map((item, idx) => (
                    <div
                        key={idx}
                        className="bg-[#f0f0f3] p-4 rounded-xl shadow-[inset_4px_4px_8px_#d1d9e6,inset_-4px_-4px_8px_#ffffff] transition-all"
                    >
...
const Education = () => {
    return (
        <section className="py-16 px-6 md:px-8 text-gray-800 max-w-6xl mx-auto">
            <div className="text-center mb-12">
                <h2 className="text-4xl font-semibold">Education & Experience</h2>
                <p className="text-sm text-gray-600 mt-2">
                    A glimpse into my background and journey so far
                </p>
            </div>
